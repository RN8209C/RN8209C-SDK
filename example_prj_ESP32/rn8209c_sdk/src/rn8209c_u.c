...
void get_user_param(STU_8209C user)
{
	...
	{
		temp = (float)(16.1079*(double)Stu8209c.KV*Stu8209c.R*4*16*100000)/Stu8209c.EC;
	...
	// rn8209c_debug(" param ku is %u\n",Stu8209c.Ku);// esp-idf 5.1 uint_32_t 格式符需用%lu
	// rn8209c_debug(" param kia is %u\n",Stu8209c.Kia);
...
uint8_t rn8209c_init_para(void )
{
    ...
    double dtemp;
    uint16_t  PSTART;
    dtemp=0.8*(double)Stu8209c.power_start/(kp*256);
    PSTART=(uint16_t )dtemp;
//写使能
    regbuf[0] = 0xE5;
    ret|=s_rn8209c_write(REG_WriteEn,regbuf,1);
    // rn8209c_debug("ku is %u,ki is %u,kIB is %u\n",Stu8209c.Ku,Stu8209c.Kia,Stu8209c.Kib);// esp-idf 5.1 uint_32_t 格式符需用%lu
